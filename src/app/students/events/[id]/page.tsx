...
export default function EventDetailPage() {
  ...
  // Student-profile iframe overlay — opens /students/[id] in-place when admin
                    ...
                    >
                      ...
                      {!hiddenCols.has('name') && (
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => setStudentProfileId(app.student_id)}
                            className={`font-medium hover:text-steps-blue-600 dark:hover:text-steps-blue-400 underline-offset-2 hover:underline text-left ${
                              app.eligibility === 'ineligible'
                                ? 'text-red-700 dark:text-red-400'
                                : 'text-gray-900 dark:text-gray-100'
                            }`}
                            title="Open student profile in a side panel"
                          >
          ...
          <div onClick={e => e.stopPropagation()} className="relative w-full max-w-4xl h-full bg-white shadow-2xl border-l border-slate-200 overflow-hidden flex flex-col">
            <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-slate-200 bg-slate-50">
              <div className="flex items-center gap-2 min-w-0">
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600">Student profile</span>
                <span className="text-sm text-slate-700">In-place — close to come back to the applicants table.</span>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <a href={`/students/${studentProfileId}`} target="_blank" rel="noopener noreferrer" className="text-xs text-slate-600 hover:text-slate-900 underline">Open in new tab</a>
                <button type="button" onClick={() => setStudentProfileId(null)} aria-label="Close" className="w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-200">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            </div>
